...
from server import orm

from server.web_framework import add_routes, add_static

from server.handlers import cookie2user, COOKIE_NAME
...
def auth_factory(app, handler):
	...
	def auth(request):
		...
		if request.path.startswith('/manage/') and (request.__user__ is None or not request.__user__.admin):
			return web.HTTPFound('/view/signin')
...
async def init(loop):
	...
	init_jinja2(app, filters = dict(datetime = datetime_filter, todatetime = todatetime_filter))
	# 添加URL处理函数, 参数handlers为模块名
	add_routes(app, 'server.handlers')
	# 添加CSS等静态文件路径
	add_static(app)
	# 启动
